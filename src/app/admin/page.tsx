      ...
      <main style={{ padding:"34px 36px", background:"#FAF9F6" }}>
        ...
        {panel === "dashboard" && (
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:28 }}>
              {[["Total Products", products.length], ["Total Orders", orders.length], ["Revenue", "₹"+revenue.toLocaleString("en-IN")], ["Avg Rating", (products.reduce((s,p)=>s+p.rating,0)/Math.max(products.length,1)).toFixed(1)]].map(([k,v]) => (
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", background:"#fff", borderRadius:14, overflow:"hidden", border:"1px solid #E7E4DC" }}>
              <thead style={{ background:"#F1EEE6" }}><tr>{["Order ID","Customer","Items","Total","Date"].map(h=><th key={h} style={{ padding:"10px 14px", textAlign:"left", fontSize:12, color:"#54585F", fontWeight:600, textTransform:"uppercase" }}>{h}</th>)}</tr></thead>
              <tbody>
                {orders.slice(0,6).map(o=>(
                  <tr key={o.id}><td style={{ padding:"10px 14px", fontSize:13 }}>{o.id}</td><td style={{ padding:"10px 14px", fontSize:13 }}>{o.customerName}</td><td style={{ padding:"10px 14px", fontSize:13 }}>{o.items?.length} item(s)</td><td style={{ padding:"10px 14px", fontSize:13 }}>₹{o.total?.toLocaleString("en-IN")}</td><td style={{ padding:"10px 14px", fontSize:13 }}>{new Date(o.createdAt).toLocaleDateString()}</td></tr>
                ))}
                {orders.length===0 && <tr><td colSpan={5} style={{ padding:40, textAlign:"center", color:"#54585F" }}>No orders yet.</td></tr>}
              </tbody>
            </table>
          </>
        )}
        ...
        {panel === "orders" && (
          <>
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", background:"#fff", borderRadius:14, overflow:"hidden", border:"1px solid #E7E4DC" }}>
              <thead style={{ background:"#F1EEE6" }}><tr>{["Order ID","Customer","Phone","Items","Total","Date"].map(h=><th key={h} style={{ padding:"10px 14px", textAlign:"left", fontSize:12, color:"#54585F", fontWeight:600, textTransform:"uppercase" }}>{h}</th>)}</tr></thead>
              <tbody>
                {orders.map(o=>(
                  <tr key={o.id}><td style={{ padding:"10px 14px", fontSize:13 }}>{o.id}</td><td style={{ padding:"10px 14px", fontSize:13 }}>{o.customerName}</td><td style={{ padding:"10px 14px", fontSize:13 }}>{o.phone}</td><td style={{ padding:"10px 14px", fontSize:13 }}>{o.items?.length}</td><td style={{ padding:"10px 14px", fontSize:13 }}>₹{o.total?.toLocaleString("en-IN")}</td><td style={{ padding:"10px 14px", fontSize:13 }}>{new Date(o.createdAt).toLocaleDateString()}</td></tr>
                ))}
